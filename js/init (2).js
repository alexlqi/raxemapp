...
$(document).ready(function(e) {
	...
	TOinit.ulTabWrap=setTimeout(function(){
		...
		if(ulTabWrap.length>0){
			...
			ulTabWrap.find("ul").css("min-width",minWidthTab+"px");
		}
	},50);
		...
		$.ajax({
			...
			data:{
				...
				data:{
					id:$("#"+t[i]).data("tabla"),
					dataset:$("#"+t[i]).data(),
					data:$("#"+t[i]).parent().find("form.filtros").serializeObject(),
				},
			},
			success: function(r){
				loader("cerrar","#"+t[i]);
...
function listarSelect(id,pointer,dataSet){
	...
	$.ajax({
		...
		data:{
			...
			data:{
				id:dataSet.tabla,
				data:$(dataSet["formfiltros"]).serializeObject(),
			},
		},
		success: function(r){
			pointer=$(pointer);
...
function listarArrDataTable(t,cols,values){
	...
	$.ajax({
		...
		data:{
			...
			data:{
				id:$("#"+t).data("tabla"),
				data:$("#"+t).parent().parent().find("form.filtros").serializeObject(),
			},
		},
		success: function(r){
