...
class permisos{
	...
	public function auth($auth=''){
		...
		$sql="call sp_auth('{$this->key}',{$this->gsp("idpanda")},'$auth');";
		$r=$this->query2arr($sql);
		if ($r["err"]) {
			...
			if($estado==0){
				switch ($tipo) {
					case 'sec':
						# code...
						$r='<script>$(document).ready(function(e){notificacion({content:"No tiene permisos para ver esta sección."});});</script>';
					break;
					case 'sub':
						# code...
						$r='<script>$(document).ready(function(e){notificacion({content:"No tiene permiso para ver este módulo."});});</script>';
					break;
					case 'tab':
						# code...
						$r='<script>$(document).ready(function(e){notificacion({content:"No tiene permiso para ver esta pestaña."});});</script>';
					break;
					case 'hashed':
	...
	public function authModel($tabla='',$pt=''){
		## si es el master entonces dale permiso
		if($this->gsp("idpanda")==0){
			return true;
		}
		## dara respuesta si no tiene permiso si no hay permiso de sms, lectura o escritura
		$sql="call sp_authModel('{$this->key}',{$this->gsp("idpanda")},'$tabla','$pt');";
		$r=$this->query2arr($sql);
		if ($r["err"]) {
			 $r='<script>$(document).ready(function(e){notificacion({content:"Hubo un error en el sistema de permisos."});});</script>';
		}elseif(!empty($r["data"])){
			...
			if($estado==0){
				switch ($tipo) {
					case 'tabla':
						# code...
						$r='<script>$(document).ready(function(e){notificacion({content:"No tiene acceso a la tabla '.$tabla.'."});});</script>';
					break;
					case 'hashed':
						$r='<script>$(document).ready(function(e){notificacion({content:"Error: la clave no es correcta."});});</script>';
					break;
	...
	## función para traer los datos de los usuarios de panda
	public function getPandas(){
		$sql="select * from pandas where idSuscripcion in (select idSuscripcion from suscripciones where hashed = '{$this->key}') and tipoUser not in (1);";
		return $this->query2arr($sql);
	}
	...
	public function navBuild($root=''){
		...
		foreach ($navArr as $k => $v){
			if($v["tipo"]=="sub"){
				#
				$navTmp["sub"][$v["seccion"]][$v["permiso"]]=array($root."{$v["seccion"]}/{$v["permiso"]}/",$v["nombre"],$v["permiso"]);
			}elseif ($v["tipo"]=="sec"){
				# 
				$navTmp["sec"][$v["permiso"]]["permiso"]=$v["permiso"];
				$navTmp["sec"][$v["permiso"]]["seccion"]=$v["nombre"];
				$navTmp["sec"][$v["permiso"]]["modulo"]=array();
			}
		}
		if(!empty($navTmp["sub"])){
			foreach ($navTmp["sub"] as $seccion => $bloque){
				$nav[$seccion]["permiso"]=@$navTmp["sec"][$seccion]["permiso"];
				$nav[$seccion]["seccion"]=@$navTmp["sec"][$seccion]["seccion"];
				$nav[$seccion]["modulo"]=$bloque;
			}
			ksort($nav);
			return $nav;
		}else{
...
function connectPermisos($dsn){
	...
	catch(PDOException $e)
	{
		...
		var_dump($_SERVER["REMOTE_ADDR"],$bd,$dsn[0]);
	}
	return $bd;
}
